...
import { PlusOutlined, EditOutlined, DeleteOutlined, LeftOutlined, RightOutlined, CaretDownOutlined, GlobalOutlined, FolderOutlined, AppstoreOutlined } from '@ant-design/icons'
...
export default function LeftSidebar({ groups, activeItemId, collapsed, onToggle, onItemSelect, onItemAdd, onItemEdit, onItemDelete, onGroupAdd }: LeftSidebarProps): React.JSX.Element {
          ...
          <div className="sidebar__header">
            ...
            <div className="sidebar__actions">
              <Tooltip title="新建分组" mouseEnterDelay={0.5}>
                <button className="sidebar__action-btn" onClick={onGroupAdd} aria-label="新建分组">
                  <PlusOutlined />
                </button>
              </Tooltip>
            </div>
                  ...
                  {!isCollapsed && (
                    <div className="sidebar__group-items">
                      {group.items.map((item) => (
                        <div key={item.id} className={`sidebar__item ${item.id === activeItemId ? 'sidebar__item--active' : ''}`} onClick={() => onItemSelect?.(item)} title={item.url || item.label}>
                          <span className="sidebar__item-icon">{item.icon || <GlobalOutlined />}</span>
