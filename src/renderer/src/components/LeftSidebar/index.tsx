...
const SortableGroup = ({
  ...
  const isActuallyCollapsed = searchText
    ? !group.title.toLowerCase().includes(searchText.toLowerCase()) &&
      !group.items.some((item) => item.name.toLowerCase().includes(searchText.toLowerCase())) &&
      isCollapsed
    : isCollapsed
  ...
  return (
    <div ref={setNodeRef} style={style} className="sidebar__group">
      <div className="sidebar__group-header" onClick={onToggle}>
        ...
        <CaretDownOutlined
          className={`sidebar__group-arrow ${isActuallyCollapsed ? 'sidebar__group-arrow--collapsed' : ''}`}
        />
        <span className="sidebar__item-icon" style={{ marginRight: 2 }}>
          {group.icon && group.icon.startsWith('data:image') ? (
            <img src={group.icon} style={{ width: 14, height: 14, borderRadius: 2 }} alt="" />
          ) : group.isApp ? (
            <AppstoreOutlined />
          ) : (
            <FolderOutlined style={{ color: group.icon && group.icon.startsWith('#') ? group.icon : 'inherit' }} />
          )}
        </span>
        ...
        {group.title !== '应用' && (
          <div className="sidebar__group-actions" style={{ display: 'flex', gap: 2 }}>
            ...
            <Tooltip title="添加子项" mouseEnterDelay={0.5}>
              <button
                className="sidebar__action-btn"
                onClick={(e) => {
                  e.stopPropagation()
...
const LeftSidebar = forwardRef<LeftSidebarRef, LeftSidebarProps>(
  ({ activeItemId, collapsed, onToggle, onItemSelect, onUpdate }, ref) => {
    ...
    const fetchBookmarks = useCallback(async () => {
      try {
        ...
        onUpdate?.() // Notify parent of potential change
      } catch (error) {
    ...
    const groups = useMemo(() => {
      // Include both Type 1 (Group) and Type 3 (App) as root items if parentId is 0
      const folders = bookmarks
        .filter((b) => (b.type === 1 || b.type === 3) && b.parentId === 0)
      ...
      return groups
        .map((group) => {
          const matchingItems = group.items.filter(
            (item) => item.name.toLowerCase().includes(term) || (item.url && item.url.toLowerCase().includes(term))
          )
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
      ...
      // 2. Reordering Items
        ...
        if (overData?.type === 'item') {
          ...
          const sameGroupItems = bookmarks
            .filter((b) => b.parentId === newParentId)
            .sort((a, b) => (a.order || 0) - (b.order || 0))
              ...
              <div className="sidebar__actions">
                <Tooltip title={isAllCollapsed ? '全部展开' : '全部收起'} mouseEnterDelay={0.5}>
                  <button
                    ...
                    aria-label={isAllCollapsed ? '全部展开' : '全部收起'}
                  >
                    <CaretDownOutlined
                      style={{ transform: isAllCollapsed ? 'rotate(-90deg)' : 'none', transition: 'transform 0.2s' }}
                    />
                  </button>
                </Tooltip>
              ...
              {filteredGroups.length === 0 ? (
                <div style={{ padding: '20px 0' }}>
                  <Empty
                    ...
                    description={searchText ? '未找到匹配内容' : '暂无分类'}
                  />
                </div>
              ) : (
